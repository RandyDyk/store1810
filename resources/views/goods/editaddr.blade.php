...
<div class="m-block-header" id="div-header">
    ...
    <a href="javascript:;" class="m-index-icon" id="btn" >修改</a>
</div>
<div class=""></div>
...
<form class="layui-form" action="">
  <div class="addrcon">
    <ul>
      <li><em>收货人</em><input type="text" class="name" placeholder="请填写真实姓名" name='address_name' value="{{$data->address_name}}" ></li>
      <li><em>手机号码</em><input type="number" class="tel" placeholder="请输入手机号" name='address_tel' value="{{$data->address_tel}}" ></li>
      <li><em>所在区域</em><input id="demo1" class="area" type="text"  name="address_area" placeholder="请选择所在区域" value="{{$data->address_area}}"></li>
      <li class="addr-detail"><em>详细地址</em><input type="text" class="content" name="address_content" placeholder="20个字以内" class="addr" value="{{$data->address_content}}"></li>
    </ul>
    @if($data->is_default==1)
    <div class="setnormal"><span>设为默认地址</span><input type="checkbox" id='checked' name="xxx" lay-skin="switch" checked > </div>
    @else
  ...
  $(function(){
    layui.use(['layer'],function(){
      ...
      $('#btn').click(function(){
        ...
        $.post(
          "{{url('goods/editaddrdo')}}",
          {obj:obj,_token:_token},
          function(res){
            if(res==1){
              layer.msg('修改地址成功',{icon:1},function(){
