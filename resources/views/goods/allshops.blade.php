    ...
    <div id="page-infinite-scroll-bottom" class="page">
        ...
        <div class="all-list-wrapper">
            ...
            <div class="menu-list-wrapper" id="divSortList">
                <ul id="sortListUl" class="list">
                    <li sortid='0' class='current' id="asd"><span class='items'><a href="{{url('goods/allshops')}}">全部商品</a></span></li>                  
                    @foreach($cate as $v)
                    <li sortid='100' reletype='1' linkaddr='' class="cateId " id="cateid" cate_id="{{$v->cate_id}}"><span class='items'>
                            <a href="{{url('goods/allshops')}}/{{$v->cate_id}}">{{$v->cate_name}}</a></span></li>
                    @endforeach
            </ul>
            </div>
            ...
            <div class="good-list-wrapper">
                <div class="good-menu thin-bor-bottom">
                    <ul class="good-menu-list" id="ulOrderBy">
                        <li orderflag="20" _type="goods_num"><a href="javascript:;">库存</a><span>↑</span></li>
                        <li orderflag="50" _type="is_new"><a href="javascript:;">新品</a><span>↑</span></li>
                        <li orderflag="30" _type="self_price"><a href="javascript:;">价格</a><span>↑</span></li>
                        <!--价值(由高到低30,由低到高31)-->
                    </ul>
                </div>
        ...
        <div class="footer clearfix">
            <ul>
                ...
                <li class="f_announced"><a href="{{url('goods/allshops')}}" class="hover"><i></i>全部商品</a></li>
                <li class="f_single"><a href="{{url('/')}}" ><i></i>最新揭晓</a></li>
        ...
        <input type="hidden" value="{{$id}}" id="cate_id">
    </div>
</div>
    ...
    $(function(){
        layui.use(['layer'],function(){
            ...
            $(document).on('click','#ulOrderBy li',function(){
                ...
                var cate_id=$("#cateid").attr('cate_id');
                // var cate_id=$("#sortListUl li[class='cateId current']").attr('cate_id');
                console.log(cate_id);
                var top=_this.children('a').next('span').text();
                
                $.ajax({
                    type:'post',
                    url:"{{url('goods/goodssort')}}",
                    data:{_type:_type,cate_id:cate_id,top:top,_token:_token},
                }).done(function(res){
                    console.log(res);
                })
            })
